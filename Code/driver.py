...
from KGEmodel import KGEModel, TransEEnhanced
...
def save_model(model, optimizer, save_variable_list, args):
    ...
    if hasattr(model, 'entity_modulus_embedding'):
        entity_modulus_embedding = torch.tensor(model.entity_modulus_embedding.weight).detach().cpu().numpy()
        np.save(
            os.path.join(args.save_path, 'entity_modulus_embedding'),
            entity_modulus_embedding
        )

    relation_embedding =  torch.tensor(model.relation_embedding.weight).detach().cpu().numpy()
    np.save(
...
def main(args):
    ...
    if args.model == 'TransEEnhanced':
        kge_model = TransEEnhanced(
