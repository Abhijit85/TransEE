...
RELATE_V_ALIASES = {'relatev', 'relate-v', 'relate_v'}
...
def _apply_relate_v_profile(parsed_args):
    model_key = str(getattr(parsed_args, 'model', '')).strip().lower()
    parsed_args.relatev_profile = model_key in RELATE_V_ALIASES
    if not parsed_args.relatev_profile:
        return

    # Treat RelateV as a named single-model profile built on top of RelatE.
    parsed_args.model = 'RelateV'

    profile_toggle = os.getenv('RELATEV_APPLY_PROFILE', '1').strip().lower()
    if profile_toggle in {'0', 'false', 'no', 'off'}:
        return

    if parsed_args.weight_decay == 5e-6:
        parsed_args.weight_decay = 3e-5
    if parsed_args.grad_clip_norm <= 0.0:
        parsed_args.grad_clip_norm = 1.0
    ...
    parsed_args.use_query_adaptive = True

    if parsed_args.phase_harmonics < 3:
        parsed_args.phase_harmonics = 3
    if parsed_args.path_loss_weight <= 0.0:
        parsed_args.path_loss_weight = 0.5
    if parsed_args.path_consistency_weight <= 0.0:
        parsed_args.path_consistency_weight = 0.1
...
def parse_args(args=None):
    ...
    _apply_relate_v_profile(parsed_args)
...
def main(args):
    ...
    if getattr(args, 'relatev_profile', False):
        logging.info(
            'RelateV profile active: lookahead=%s ema=%s grad_clip=%.3f relation_gate=%s '
            'hierarchy_mod_head=%s query_adaptive=%s phase_harmonics=%d path_loss_weight=%.3f',
            args.use_lookahead,
            args.use_ema,
            args.grad_clip_norm,
            args.use_relation_gate,
            args.use_hierarchy_mod_head,
            args.use_query_adaptive,
